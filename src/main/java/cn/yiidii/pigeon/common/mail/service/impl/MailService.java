...
public class MailService implements IMailService {
    ...
    public void sendHtmlMail(String subject, String content, String[] toWho) {
        ...
        log.info("开始发送模板邮件：主题->{}，收件人->{}", subject, toWho);
        MimeMessage mimeMessage = mailSender.createMimeMessage();
        try {
            MimeMessageHelper helper = new MimeMessageHelper(mimeMessage, true, DEFAULT_ENCODING);
            //设置邮件的基本信息
    ...
    public void sendTemplateHtmlMail(String subject, String content, String[] toWho, String templateFileName, Object model) {
        ...
        log.info("开始发送模板Html邮件：主题->{}，收件人->{}", subject, toWho);
        //html
        MimeMessage mimeMessage = mailSender.createMimeMessage();
        try {
            MimeMessageHelper helper = new MimeMessageHelper(mimeMessage, true, DEFAULT_ENCODING);
    ...
    public void handleBasicInfo(MimeMessageHelper mimeMessageHelper, String subject, String content, String[] toWho, String templateName, Object model) {
        try {
            ...
            mimeMessageHelper.setSubject(subject);
            //设置邮件的内容
            if (StringUtils.isNotBlank(templateName) && null != model) {
                mimeMessageHelper.setText(praseTemplateContent(templateName, model), true);
            } else {
    ...
    private String praseTemplateContent(String templateName, Object model) {
        try {
            //创建配置类
